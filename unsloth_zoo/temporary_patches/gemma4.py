...
def patch_Gemma4RMSNorm():
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        if input_dtype == torch.float16:
            ...
            output_fp32 = torch.clamp(output_fp32, min=-fp16_max, max=fp16_max)
...
def patch_Gemma4TextScaledWordEmbedding():
    ...
    def forward(self, input_ids: torch.Tensor):
        ...
        # then return in weight dtype with clamping for fp16.
        input_dtype = self.weight.dtype
        result_fp32 = input_embeds.to(torch.float32) * self.embed_scale
        if input_dtype == torch.float16:
            fp16_max = torch.finfo(torch.float16).max
            result_fp32 = torch.clamp(result_fp32, min=-fp16_max, max=fp16_max)
        return result_fp32.to(input_dtype)
    pass
...
def patch_Gemma4TextAttention():
    ...
    def forward_function(
        ...
        **kwargs: KWARGS_TYPE,
        ...
        attn_output = self.o_proj(attn_output_fp32.to(self.o_proj.weight.dtype))
...
def patch_Gemma4TextMLP():
    ...
    def forward(self, x):
        ...
        down_proj_out = self.down_proj(intermediate_fp32.to(self.down_proj.weight.dtype))
...
def patch_Gemma4TextDecoderLayer():
    ...
    def forward(self, hidden_states, per_layer_input=None, position_embeddings=None,
        ...
        # Ensure hidden_states matches weight dtype before entering the decoder layer.
        # During gradient checkpointing backward recompute, it may arrive as fp32.
        _target_dtype = self.input_layernorm.weight.dtype if hasattr(self, "input_layernorm") else torch.float16
        ...
        if per_layer_input is not None and per_layer_input.dtype == torch.float32:
            per_layer_input = per_layer_input.to(_target_dtype)
